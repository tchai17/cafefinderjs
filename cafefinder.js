...
function askQuestion(query) {
    ...
    process.stdin.resume(); // Resume stdin to allow for user input
...
async function saveTitlesToFile(titles, fileName) {
    try {
        ...
        console.log(`Titles saved to ${fileName}`);
    } catch (error) {
...
async function testfunc() {
    ...
    try {
        // go to website
        await driver.get("https://eatbook.sg/search/");

        // Close any additional windows that might have opened
        ...
        // Wait for 'All Articles' tab to be located and visible
        const allTab = By.id("all-tab");
